...
class Process extends CI_Controller
{
    ...
    public function index()
    {
        ...
        //Term & Condition
        $data['term_condition'] = $this->Model_crud->select_where('page', array(
            "page_id" => 4
        ));
        ...
        /* Biling Detail Start */
        //Data User
        $data['user'] = $this->Model_crud->select_where('customer', array(
            'customer_id' => $this->session->userdata('uid')
        ));
        ...
        $data['zones'] = $this->Model_crud->select_where('zone', array(
        ...
        // Data Shipping
        $data['shipping'] = $this->Model_crud->select_where('setting', array(
            'code' => 'shipping'
        ));
        
        /* Payment Start */
        // Bank
        $data['bank'] = $this->Model_crud->select_where('setting', array(
            'setting_id' => 1
        ));
        
        // Paypal
        $data['paypal'] = $this->Model_crud->select_where('setting', array(
            'setting_id' => 2
        ));
        /* Payment End */
        ...
        if ($this->session->userdata('value_shipping_country_id') != NULL) {
            ...
            $data['shipping_sess'] = $this->Model_crud->select_where('country', array(
                'country_id' => $this->session->userdata('value_shipping_country_id')
            ));
    ...
    public function save()
    {
        ...
        if (!empty($shipping_country_id)) {
            $shipping_query = $this->Model_crud->select_where('country', array(
                            ...
                            foreach ($product_info as $index => $value) {
                                ...
                                if (is_array($row['option']) && !empty($row['option'])) {
                                    ...
                                    foreach ($row['option'] as $index => $value) {
                                        $query_option = $this->Model_crud->select_where('product_option_value', array(
        ...
        if (count($json) > 0) {
            foreach ($json as $row) {
                ...
                $ex_update      = $this->Model_crud->update('product', $data_update, array(
                    'product_id' => $products[0]['product_id']
                ));
            }
        }
        ...
        $country_name = $this->Model_crud->select_where('country', array(
            'country_id' => $country_id
        ));
        $zone_name    = $this->Model_crud->select_where('zone', array(
            'zone_id' => $zone_id
        ));
        ...
        // Email to Merchant
        $store_mail = $this->Model_crud->select_where('setting', array(
            'setting_id' => 19
        ));
    ...
    private static function delete_asso_arr($arr = array(), $key_indicator, $val_indicator)
    {
