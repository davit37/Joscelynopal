...
class Process extends CI_Controller
{
            ...
            if (!empty($check_order))
            {
                if ($check_order[0]['order_status'] == 'Completed')
                {
                        ...
                        ->userdata('uid'))
                    {
                        ...
                        $this
                            ->Model_crud
                            ->update('customer', $data_update, array(
                            'customer_id' => $this
                                ->session
                                ->userdata('uid')
                        ));
    ...
    public function index()
    {
        ...
        $data['category'] = $this
            ->Model_crud
            ->select_where_order('category', array(
            "status" => 1
        ) , 'sort_order', 'asc');

        //Term & Condition
        $data['term_condition'] = $this
            ->Model_crud
            ->select_where('page', array(
            "page_id" => 4
        ));
            ...
            foreach ($json as $row)
            {
                ...
                if (!empty($result_json))
                {
                    ...
                    foreach ($result_json as $index => $value)
                    {
                        ...
                        //UNSET PRODUCT SOLD OUT
                        if($value['sales_status'] == "sold_out"){
                            $reset_cart =true;
                            $new_json = self::delete_asso_arr($json, "product_id",$value['product_id'] );
                            $json = $new_json;
        ...
        else
        {
            ...
            $this->session->set_flashdata('message', 'you can\'t process cart is empty');
            redirect('home');                       
        }
        ...
        if ($this
            ->session
            ->userdata('value_shipping_country_id') != NULL)
        {

            $data['shipping_sess'] = $this
                ->Model_crud
                ->select_where('country', array(
                'country_id' => $this
                    ->session
                    ->userdata('value_shipping_country_id')
            ));
    ...
    public function save()
    {
        ...
        $shipping_method = $this
            ->input
            ->post('shipping_method', true);
        $shipping_comment = $this
            ->input
            ->post('shipping_comment', true);
        $shipping_country_id = $this
            ->input
            ->post('shipping_country_id', true);
        $payment_method = $this
            ->input
            ->post('payment_method', true);
        $payment_comment = $this
            ->input
            ->post('payment_comment', true);
        ...
        if ($agree == '')
        {
            $this
                ->session
                ->set_userdata('error_agree', 'Warning: You must agree to the Terms & Conditions!');
            $error = true;
        }
        ...
        $this
            ->session
            ->set_userdata('value_firstname', $firstname);
        $this
            ->session
            ->set_userdata('value_lastname', $lastname);
        $this
            ->session
            ->set_userdata('value_email', $email);
        $this
            ->session
            ->set_userdata('value_telephone', $telephone);
        $this
            ->session
            ->set_userdata('value_fax', $fax);
        $this
            ->session
            ->set_userdata('value_company', $company);
        $this
            ->session
            ->set_userdata('value_address_1', $address_1);
        $this
            ->session
            ->set_userdata('value_address_2', $address_2);
        $this
            ->session
            ->set_userdata('value_city', $city);
        $this
            ->session
            ->set_userdata('value_postcode', $postcode);
        $this
            ->session
            ->set_userdata('value_country_id', $country_id);
        $this
            ->session
            ->set_userdata('value_zone_id', $zone_id);
        $this
            ->session
            ->set_userdata('value_shipping_country_id', $shipping_country_id);
        $this
            ->session
            ->set_userdata('value_shipping_method', $shipping_method);
        $this
            ->session
            ->set_userdata('value_shipping_comment', $shipping_comment);
        $this
            ->session
            ->set_userdata('value_payment_method', $payment_method);
        $this
            ->session
            ->set_userdata('value_payment_comment', $payment_comment);
        ...
        $this
            ->session
            ->set_userdata('value_order_id', $order_id);
                    ...
                    foreach ($row['option'] as $index => $value)
                    {
                        ...
                        if (!empty($data_option))
                        {
                            ...
                            foreach ($data_option as $k => $values)
                            {
                                ...
                                $ex_ins = $this
                                    ->Model_crud
                                    ->insert('order_option', $data_option);
    ...
    function _sendMail($seg1 = '', $seg2 = '', $seg3 = '', $seg4 = '', $seg5 = '', $seg6 = '')
    {
        ...
        $message = $this
            ->load
            ->view('email/catalog_order', $data, true);
    ...
    private static function delete_asso_arr($arr=[],$key_indicator, $val_indicator ){
        ...
        foreach($arr as $key => $val){
            if($val[$key_indicator] == $val_indicator){
                unset($arr[$key]);
            }
        }
