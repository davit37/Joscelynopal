...
class Order extends CI_Controller {
    ...
    public function index() {
        ...
        if(!empty($data['results'])){
            ...
            foreach($data['results'] as $index => $value){
                $query_total = $this->Model_crud->select_where('country',array('country_id' => $value['shipping_country_id']));
        ...
        $config["total_rows"] = $this->Model_crud->total_row('order');
    ...
    public function autocomplete($seg1 = '', $seg2 = '') {
        ...
        switch ($seg1) {
            ...
            $like_by = $seg2;
            $results = $this->Model_crud->select_like_limit('order', array("$like" => $like_by), $limit);
    ...
    public function filter($seg1 = '', $seg2 = '') {
        ...
        $order_status = $this->input->post('filter_order_status');
        $total = $this->input->post('filter_total');
        $date_added = $this->input->post('filter_date_added');
        $date_modified = $this->input->post('filter_date_modified');
        $query_where = array();
        if ($order_id != '') {
        ...
        switch ($seg1) {
            ...
            foreach ($results as $result) {
                $json[] = array(
                    ...
                    'zones' => $this->Model_crud->select_where('zone', array('country_id'=>$result['country_id']))
                    );
            }
            break;
    ...
    public function product_add($seg1 = '') {
        ...
        $product_info = $this->Model_crud->select_where('product', array('product_id' => $product_id));
        if ($product_info) {
            /*$quantity = $seg2;
            if (isset($quantity)) {
                $quantity = (int) $seg2;
            ...
            //Get Cart
            if ($this->session->userdata('admin_cart')) {
                $json = json_decode($this->session->userdata('admin_cart'), true);
            } else {
                $this->session->set_userdata('admin_cart', json_encode($json));
            }
            ...
            foreach ($json as $row) {
                ...
                $category = $this->Model_crud->select_where('category', array('category_id'=>$product_info[0]['category_id']));
                $special_price = $this->Model_crud->select_where('product_special', array('product_id'=>$product_id));
                $query_option  = $this->Model_backend->get_option_and_value_by_id($product_id);
                if($special_price) {
        ...
        public function option_value_change($seg1 = ''){
            ...
            $query = $this->Model_crud->select_where('product_option_value',array('product_option_id' => $product_option_id));
        ...
        public function option_change($seg1 = '',$seg2 = ''){
            ...
            $query_product_option_value       = $this->Model_backend->get_product_option_change($product_option_value,$product_id);
        ...
        public function option_add($seg1 = ''){
            ...
            if($_POST){
                ...
                $product_option_value   = $this->input->post('product_option_value');

                $query_product_option       = $this->Model_backend->get_product_option_add($product_option,$product_option_value,$product_id);
                $query_product              = $this->Model_backend->get_product_and_category($product_id);
                $query_shipping_country     = $this->Model_backend->get_shipping_price_from_order($order_id);
                $query_product_option_value = $this->Model_crud->select('product_option_value');
...
//Rebuilt Cart
            foreach ($json as $row) {
                if ($row['product_id'] != $product_id) {
                    $product_info = $this->Model_crud->select_where('product', array('product_id' => $row['product_id']));
                    $category = $this->Model_crud->select_where('category', array('category_id'=>$product_info[0]['category_id']));
...
//Update cart
            $this->session->set_userdata('admin_cart', json_encode($new_json));
        ...
        public function save() {
            ...
            $shipping_comment = $this->input->post('shipping_comment');
...
//Order Details
            ...
            if(!empty($data['order_detail'])){
                $data['order_shipping_country'] = $this->Model_crud->select_where('country',array('country_id' => $data['order_detail'][0]['shipping_country_id']));
            }
//Order History
            $data['order_history'] = $this->Model_crud->select_where('order_history', array('order_id'=>$order_id));
            $total_row_order_history = $this->Model_crud->total_row_where('order_history', array('order_id'=>$order_id));
...
//View
        ...
        public function notify() {
            ...
            $ex_upd = $this->Model_crud->update('order', $data_update, array('order_id'=>$order_id));
...
//Get Order
                $order = $this->Model_crud->select_where('order', array('order_id'=>$order_id));
                $country_name = $this->Model_crud->select_where('country', array('country_id'=>$order[0]['country_id']));
                $zone_name = $this->Model_crud->select_where('zone', array('zone_id'=>$order[0]['zone_id']));
                $data_email = array(
                    'order_id' => $order_id,
...
//Get Order Product
                $products = $this->Model_crud->select_where('order_product', array('order_id'=>$order_id));

//Get Order Total
                $total = $this->Model_crud->select_where('order_total', array('order_id'=>$order_id));

//Get Order History
                $order_history = $this->Model_crud->select_where('order_history', array('order_id'=>$order_id));

                $this->_sendMail($order[0]['email'], $title, $data_email, $products, $total[0]['value'], $order_history);
            }
...
//Store Config
            $data['config_store_name'] = $this->Model_crud->select_where('setting', array('setting_id'=>17));
            $data['config_store_address'] = $this->Model_crud->select_where('setting', array('setting_id'=>18));
            $data['config_store_email'] = $this->Model_crud->select_where('setting', array('setting_id'=>19));
            $data['config_store_telephone'] = $this->Model_crud->select_where('setting', array('setting_id'=>20));

//View
            $this->load->view('admin/sale/order/order_invoice', $data);
        }
...
//Customer
            if($pdata['order_detail'][0]['customer_id'] == 0){
                ...
                $data['customer'] = $this->Model_crud->select_where('customer',array('customer_id'=>$data['order_detail'][0]['customer_id']));
            }

//Country
            $data['product'] = $this->Model_crud->select('product');

//Country
            $data['countries'] = $this->Model_crud->select('country');

//Zone
            $data['zones'] = $this->Model_crud->select_where('zone',array('country_id'=>$data['order_detail'][0]['country_id']));

//Order Product
            $data['order_products'] = $this->Model_crud->select_where('order_product', array('order_id'=>$order_id));

            $data['product_option'] = $this->Model_crud->select('product_option');

//Order Option
            if(!empty($data['order_products'])){
                foreach($data['order_products'] as $index => $value){
                    $order_option = $this->Model_backend->get_order_option($value['order_id'],$value['product_id']);
                    $special_price = $this->Model_crud->select_where('product_special', array('product_id'=>$value['product_id']));
                    if($special_price) {
                        $now = strtotime(date('Y-m-d'));
                        $date_end = strtotime($special_price[0]['date_end']);
                        if($date_end >= $now ) {
                    ...
                    foreach($order_option as $key => $row){
                        if($value['product_id'] == $row['order_product_id']){
                            ...
                            $data['order_products'][] = array($value);
                        }
                    }
                }
            }
...
//Order Product Option Value
            $data['product_option_value'] = $this->Model_crud->select('product_option_value');

//Order Shipping
            $data['shipping_country'] = $this->Model_crud->select_where('country',array('country_id'=>$data['order_detail'][0]['shipping_country_id']));
...
###Delete Old Data & Insert Data Order Product###
    $old_carts = $this->Model_crud->select_where('order_product', array("order_id"=>$order_id));
//Realising old cart qty
    foreach ($old_carts as $old_cart) {
        ...
        $ex_upd = $this->Model_crud->update('product', $data_update, array('product_id'=>$old_cart['product_id']));
    }

    $ex_del = $this->Model_crud->delete('order_product', array("order_id"=>$order_id));
...
//Update Stock Quantity
            ...
            $ex_upd = $this->Model_crud->update('product', $data_update, array('product_id'=>$products[0]['product_id']));
        }
    }
    /* Get Cart End */
...
//Insert Data Order Total
    ...
    $ex_upd = $this->Model_crud->update('order_total', $data_update, array("order_id" => $order_id));

    $this->session->set_userdata('order_success', 'Success: You have modified order!');
...
//Delete table order
    $ex_del = $this->Model_crud->delete('order', array('order_id'=>$order_id));

//Delete table order history
    $ex_del = $this->Model_crud->delete('order_history', array('order_id'=>$order_id));

//Delete table order product
    $ex_del = $this->Model_crud->delete('order_product', array('order_id'=>$order_id));

//Delete table order total
    $ex_del = $this->Model_crud->delete('order_total', array('order_id'=>$order_id));
...
// message
    ...
    $message = $this->load->view('admin/email/order_history',$data,TRUE);
