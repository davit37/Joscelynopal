...
class Product_option extends CI_Controller {
    ...
    public function form() {
        ...
        if ($action == 'edit') {
            ...
            $data['result'] = $this->Model_crud->select_where('p_option', array(
                "id" => $id
            ));
            $json_child     = $data['result'][0]['json_child'];
            
            //make option list html
            if ($json_child != null) {
                ...
                foreach ($data['json_child'] as $key => $value) {
                    ...
                    $data['json_child'][$key]['child_name_html'] = '<input type="text"  value="' . $value["child_name"] . '" class="form-control option-name" data-option-index="' . $key . '"' . $parent_id . '>';
    ...
    public function delete() {
        ...
        for ($i = 0; $i < count($checkbox); $i++) {
            $ex_del = $this->Model_crud->delete('p_option', array(
                "id" => $checkbox[$i]
            ));
        }
